...
export async function checkoutV101(
	...
	const { netAmount } = getNetAmountAndFeesV101(amountDecimal, project);
		...
		transfer = await prisma.$transaction(
			async (tx): Promise<Mangopay.transfer.TransferData> => {
				...
				if (amountCrowdfunded.lt(netAmount)) {
					amountDecimal = getGrossAmountDecimal(
						amountCrowdfunded,
						project
					);
				}
