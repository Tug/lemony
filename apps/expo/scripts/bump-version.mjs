...
try {
	...
	console.log(
		`The starting working directory is ${chalk.blue(
			startingWorkingDirectory
		)}`
	);
	console.log(
		`The current working directory is ${chalk.blue(
	...
	if (versionType === 'major') {
		...
		if (hasPossiblePackageChanges) {
			...
			const reactNativeConfig = JSON.parse(
				reactNativeConfigResponse.stdout
			);

			const hasIos = hasPlatform(fileDiffSanitized, reactNativeConfig, {
				ios: true,
			});
			const hasAndroid = hasPlatform(
				fileDiffSanitized,
				reactNativeConfig,
				{
					android: true,
