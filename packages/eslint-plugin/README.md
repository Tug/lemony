...
```bash
npm install @diversifiedfinance/eslint-plugin --save-dev
```
...
```json
{
	"extends": ["plugin:@diversifiedfinance/eslint-plugin/recommended"]
}
```
...
```json
{
	"extends": ["plugin:@diversifiedfinance/eslint-plugin/esnext"]
}
```
...
| [no-unsafe-wp-apis](https://github.com/WordPress/gutenberg/tree/HEAD/packages/eslint-plugin/docs/rules/no-unsafe-wp-apis.md)                                         | Disallow the usage of unsafe APIs from `@diversifiedfinance/*` packages                                    | ✓           |
...
```json
{
	"extends": ["plugin:@diversifiedfinance/eslint-plugin/jshint"]
}
```
