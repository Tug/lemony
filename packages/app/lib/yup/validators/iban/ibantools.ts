// From: https://github.com/Simplify/ibantools/blob/master/src/ibantools.ts
...
export function validateIBAN(
	...
	if (iban !== undefined && iban !== null && iban !== '') {
		...
		if (
			spec &&
			spec.bban_validation_func &&
			!spec.bban_validation_func(iban.slice(4))
		) {
			result.valid = false;
		...
		if (
			result.errorCodes.indexOf(ValidationErrorsIBAN.WrongBBANFormat) !==
				-1 ||
			!isValidIBANChecksum(iban)
		) {
			result.valid = false;
...
const checkNorwayBBAN = (bban: string): boolean => {
	...
	const bbanWithoutControlDigit = bbanWithoutSpacesAndPeriods.substring(
		0,
		10
	);
	let sum = 0;
	for (let index = 0; index < 10; index++) {
		sum +=
			parseInt(bbanWithoutControlDigit.charAt(index), 10) *
			weights[index];
	}
...
const checkSpainBBAN = (bban: string): boolean => {
	...
	for (let index = 0; index < 8; index++) {
		sum +=
			parseInt(bankBranch.charAt(index), 10) * weightsBankBranch[index];
	}
...
const checkCroatianBBAN = (bban: string): boolean => {
	...
	return (
		checkMod1110(bankBranch, controlBankBranch) &&
 ...
 * @param func
 */
export const setCountryBBANValidation = (
	country: string,
