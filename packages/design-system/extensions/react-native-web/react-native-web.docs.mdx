---
description: 'A standard React Native component development environment'
labels: ['react-native', 'typescript', 'env', 'extension']
---

## Overview

A customized version which extends the default React Native component development environment created by teambit. This environment can be applied to all your components or a set of components under the variants of your `workspace.json` file. That means they will use your custom environment instead of the default environment. Environment components are just like any other Bit components in that they can be exported and then shared and used in various projects which makes it easier to create standards when working with many teams.

### Usage instructions

Under the **variant** section of your `workspace.json` file choose which components you want to have the custom environment set. You can find the id of the extension in the `react-native-web.aspect.ts` file.
...
Component testing is done through [Jest](https://jestjs.io/) with the default `teambit.react/react-native` environment. You can modify the `jestconfig.js` file to add your own configurations which will then be merged with the default configs set by teambit.
...
```js
const reactJestConfig = require('@teambit/react-native/jest/jest.config');
module.exports = {
	...reactJestConfig,
...
Bit uses ESLint to lint your components. You can add your own rules in the `react-native-web.main.runtime` file.
...
Bit uses Prettier to format your components. You can add your own rules in the `react-native-web.main.runtime` file.
...
Override the default dependencies in the `react-native-web.main.runtime` file to include react types of a different version for example.
...
Extend the `react-native-web.preview.runtime` file when you want to add your own customizations only for previewing in the Bit UI.
